...
const globeLocations = [
  { key: "seattle", en: "Seattle", zh: "西雅图", lat: 47.61, lon: -122.33 }, { key: "sammamish", en: "Sammamish", zh: "萨马米什", lat: 47.62, lon: -122.04 }, { key: "chelan", en: "Chelan", zh: "奇兰", lat: 47.84, lon: -120.02 },
  { key: "alaska", en: "Alaska", zh: "阿拉斯加", lat: 61.22, lon: -149.9 }, { key: "hawaii", en: "Hawaii", zh: "夏威夷", lat: 21.31, lon: -157.86 }, { key: "canada", en: "Vancouver", zh: "温哥华", lat: 49.28, lon: -123.12 },
  { key: "yellowstone", en: "Yellowstone", zh: "黄石", lat: 44.43, lon: -110.59 }, { key: "chicago", en: "Chicago", zh: "芝加哥", lat: 41.88, lon: -87.63 }, { key: "harbin", en: "Harbin", zh: "哈尔滨", lat: 45.8, lon: 126.53 },
  { key: "korea", en: "Seoul", zh: "首尔", lat: 37.57, lon: 126.98 }, { key: "japan", en: "Hokkaido", zh: "北海道", lat: 43.06, lon: 141.35 },
];
...
export default function Home() {
  ...
  return (
    <main lang={language === "zh" ? "zh-CN" : "en"}>
      ...
      <section className="bookshelf section-pad" id="books"><div className="books-heading"><div><p className="eyebrow">{language === "zh" ? "博南书单" : "Bonan’s bookshelf"}</p><h2>{language === "zh" ? "那些改变我提问方式的书。" : "Books that changed the way I ask questions."}</h2></div><p>{language === "zh" ? "不是排行榜，而是一组值得反复翻回去的思想工具。每一本，都留下了一个我仍在使用的问题。" : "Not a ranking, but a working shelf of ideas worth returning to—each one leaving behind a question I still use."}</p></div><div className="book-grid">{books.map((book) => <article className="book-card" key={book.isbn}><img src={`https://covers.openlibrary.org/b/isbn/${book.isbn}-L.jpg`} alt={`${book.title} cover`} loading="lazy"/><div><small>{book.author}</small><h3>{book.title}</h3><p>{book[language]}</p></div></article>)}</div><Engagement id="books" language={language}/></section>
