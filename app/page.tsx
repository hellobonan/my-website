  ...
  return (
    <main lang={language === "zh" ? "zh-CN" : "en"}>
      ...
      <section className="essay-salon section-pad" id="essays">
        ...
        <div className="essay-grid">{filteredEssays.map((essay, index) => { const originalIndex = Number(essay.id.slice(-2)) - 1; const englishStory = language === "en" ? essay as EnglishStory : null; return <button type="button" onClick={() => setSelectedEssayId(essay.id)} className={`essay-card essay-tone-${(originalIndex % 6) + 1}`} key={essay.id}><div className="essay-art">{englishStory?.image ? <><img src={englishStory.image} alt="" loading="lazy"/><time className="essay-date" dateTime={englishStory.date}><b>{new Date(`${englishStory.date}T12:00:00`).toLocaleString("en", { month: "short" })}</b><span>{new Date(`${englishStory.date}T12:00:00`).getDate()}</span><i>{new Date(`${englishStory.date}T12:00:00`).getFullYear()}</i></time></> : <span>{String(index + 1).padStart(2, "0")}</span>}</div><div className="essay-card-copy"><small>{essayCategories[language][essay.category as keyof typeof essayCategories.en]}{englishStory ? ` | Published ${englishStory.dateLabel}` : ""}</small><h3>{englishStory ? englishStory.title : essay.title}</h3><p>{englishStory ? englishStory.summary : (essay as Essay).excerpt}</p><span>{language === "zh" ? "阅读全文" : "Read full story"} →</span></div></button>; })}</div><Engagement id="essays" language={language}/><SectionHome language={language}/>
      </section>
      ...
      <section className="about section-pad" id="about"><div className="about-portrait"><img src="/media/linkedin-profile.jpg" alt={language === "zh" ? "博南钟的职业肖像" : "Professional portrait of Bonan Zhong"}/></div><div className="about-copy"><p className="eyebrow">{t.about}</p><h2>{t.aboutTitle}</h2><p className="about-lede">{t.aboutLede}</p><div className="about-columns"><p>{t.aboutA}</p><p>{t.aboutB}</p></div>
