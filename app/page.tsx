...
export default function Home() {
  ...
  return (
    <main lang={language === "zh" ? "zh-CN" : "en"}>
      ...
      {selectedEssay && <div className="reader-overlay" role="presentation" onMouseDown={() => setSelectedEssayId(null)}><article className="reader-panel essay-reader" role="dialog" aria-modal="true" aria-labelledby="essay-reader-title" onMouseDown={(event) => event.stopPropagation()}><div className="reader-topline"><div><span>{essayCategories[language][selectedEssay.category as keyof typeof essayCategories.en]}</span><span>{language === "zh" ? "原创中文" : "FULL ENGLISH TRANSLATION"}</span></div><button type="button" onClick={() => setSelectedEssayId(null)}>{t.close} ×</button></div><div className="reader-body"><p className="essay-reader-kicker">{language === "zh" ? "人间拾光" : "Collected Light"}</p><h2 id="essay-reader-title">{language === "zh" ? selectedEssay.title : (selectedEssayTranslation?.title || essayEnglishTitles[Number(selectedEssay.id.slice(-2)) - 1])}</h2><div className="reader-copy">{(language === "zh" ? selectedEssay.body : selectedEssayTranslation?.body || "").split(/\n\s*\n/).filter(Boolean).map((paragraph, index) => <p key={`${selectedEssay.id}-${language}-${index}`}>{paragraph}</p>)}</div><Engagement id={selectedEssay.id} language={language}/></div><div className="reader-footer"><span>Hello Bonan</span><button type="button" onClick={() => setSelectedEssayId(null)}>{language === "zh" ? "返回人间拾光" : "Back to Collected Light"}</button></div></article></div>}
