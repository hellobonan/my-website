  ...
  return (
    <main lang={language === "zh" ? "zh-CN" : "en"}>
      ...
      <section className="essay-salon section-pad" id="essays">
        ...
        <div className="essay-toolbar"><div className="essay-filters" aria-label={language === "zh" ? "随笔分类" : "Story categories"}>{Object.entries(essayCategories[language]).map(([key, label]) => <button key={key} type="button" className={essayCategory === key ? "active" : ""} onClick={() => setEssayCategory(key)}>{label}<span>{key === "all" ? activeEssays.length : activeEssays.filter((essay) => essay.category === key).length}</span></button>)}</div>{language === "en" && <label className="essay-sort"><span>Sort by date</span><select value={essaySort} onChange={(event) => setEssaySort(event.target.value)}><option value="newest">Newest first</option><option value="oldest">Oldest first</option></select></label>}</div>
        <div className="essay-grid">{filteredEssays.map((essay, index) => { const originalIndex = Number(essay.id.slice(-2)) - 1; const englishStory = language === "en" ? essay as EnglishStory : null; return <button type="button" onClick={() => setSelectedEssayId(essay.id)} className={`essay-card essay-tone-${(originalIndex % 6) + 1}`} key={essay.id}><div className="essay-art">{englishStory?.image ? <img src={englishStory.image} alt="" loading="lazy"/> : <span>{String(index + 1).padStart(2, "0")}</span>}</div><div className="essay-card-copy"><small>{essayCategories[language][essay.category as keyof typeof essayCategories.en]}{englishStory ? ` ? ${englishStory.dateLabel}` : ""}</small><h3>{englishStory ? englishStory.title : essay.title}</h3><p>{englishStory ? englishStory.summary : essay.excerpt}</p><span>{language === "zh" ? "????" : "Read full story"} ?</span></div></button>; })}</div><Engagement id="essays" language={language}/>
      </section>
