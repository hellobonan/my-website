...
const allBooks = [...featuredBooks, ...libraryBooks];

function BookCover({ title, isbn }: { title: string; isbn?: string }) { const [src, setSrc] = useState(isbn ? `https://covers.openlibrary.org/b/isbn/${isbn}-L.jpg` : ""); useEffect(() => { if (isbn) return; let active = true; fetch(`https://openlibrary.org/search.json?title=${encodeURIComponent(title)}&limit=1&fields=cover_i`).then((response) => response.json()).then((data) => { if (active && data.docs?.[0]?.cover_i) setSrc(`https://covers.openlibrary.org/b/id/${data.docs[0].cover_i}-L.jpg`); }).catch(() => {}); return () => { active = false; }; }, [title, isbn]); return src ? <img src={src} alt={`${title} cover`} loading="lazy"/> : <div className="book-cover-fallback"><span>{title}</span><i>Hello Bonan Library</i></div>; }
...
const globeLocations = [
  { key: "seattle", en: "Seattle", zh: "西雅图", lat: 47.61, lon: -122.33, landmark: "Space Needle, Seattle" }, { key: "sammamish", en: "Sammamish", zh: "萨马米什", lat: 47.62, lon: -122.04, landmark: "Lake Sammamish, Washington" }, { key: "chelan", en: "Chelan", zh: "奇兰", lat: 47.84, lon: -120.02, landmark: "Lake Chelan, Washington" },
  { key: "alaska", en: "Alaska", zh: "阿拉斯加", lat: 61.22, lon: -149.9, landmark: "Denali, Alaska" }, { key: "hawaii", en: "Hawaii", zh: "夏威夷", lat: 21.31, lon: -157.86, landmark: "Diamond Head, Hawaii" }, { key: "canada", en: "Vancouver", zh: "温哥华", lat: 49.28, lon: -123.12, landmark: "Canada Place, Vancouver" }, { key: "toronto", en: "Toronto", zh: "多伦多", lat: 43.65, lon: -79.38, landmark: "CN Tower, Toronto" },
  { key: "yellowstone", en: "Yellowstone", zh: "黄石", lat: 44.43, lon: -110.59, landmark: "Yellowstone bison, Lamar Valley" }, { key: "chicago", en: "Chicago", zh: "芝加哥", lat: 41.88, lon: -87.63, landmark: "Cloud Gate, Chicago" }, { key: "harbin", en: "Harbin", zh: "哈尔滨", lat: 45.8, lon: 126.53, landmark: "Saint Sophia Cathedral, Harbin" },
  { key: "korea", en: "Seoul", zh: "首尔", lat: 37.57, lon: 126.98, landmark: "Gyeongbokgung Palace, Seoul" }, { key: "japan", en: "Hokkaido", zh: "北海道", lat: 43.06, lon: 141.35, landmark: "Furano flower fields, Hokkaido" },
];
...
export default function Home() {
  ...
  const filteredBooks = allBooks.filter((book) => `${book.title} ${book.author} ${bookTheme(book.title)}`.toLowerCase().includes(bookQuery.toLowerCase()));
  ...
  return (
    <main lang={language === "zh" ? "zh-CN" : "en"}>
      ...
      <section className="postcards section-pad" id="postcards"><div className="section-heading postcards-heading"><div><p className="eyebrow">{t.postcards}</p><h2>{t.postcardsTitle}</h2></div><p>{t.postcardsText}</p></div>
        <div className="atlas-map"><div className="globe-stage"><div className="globe-controls"><button type="button" onClick={() => setGlobeZoom((z) => Math.min(1.28, z + .1))} aria-label="Zoom in">＋</button><button type="button" onClick={() => setGlobeZoom((z) => Math.max(.8, z - .1))} aria-label="Zoom out">−</button><small>{language === "zh" ? "拖拽旋转地球 · 滚轮缩放" : "DRAG THE EARTH · SCROLL TO ZOOM"}</small></div><div className="globe" style={{ transform: `scale(${globeZoom})`, backgroundPosition: `${globeRotation * 4}px center` }} onPointerDown={(event) => { dragStart.current = { x: event.clientX, rotation: globeRotation }; event.currentTarget.setPointerCapture(event.pointerId); }} onPointerMove={(event) => { if (dragStart.current) setGlobeRotation(dragStart.current.rotation + (event.clientX - dragStart.current.x) * .45); }} onPointerUp={() => { dragStart.current = null; }} onWheel={(event) => { event.preventDefault(); setGlobeZoom((z) => Math.max(.8, Math.min(1.28, z - event.deltaY * .001))); }}><div className="globe-grid"/>{globeLocations.map((place) => { const lambda = (place.lon + globeRotation) * Math.PI / 180; const phi = place.lat * Math.PI / 180; const visible = Math.cos(phi) * Math.cos(lambda) > -.08; const x = 50 + Math.cos(phi) * Math.sin(lambda) * 43; const y = 50 - Math.sin(phi) * 43; return <button key={place.key} type="button" className={`globe-pin ${mapArea === place.key ? "active" : ""}`} style={{ left: `${x}%`, top: `${y}%`, opacity: visible ? 1 : 0, pointerEvents: visible ? "auto" : "none" }} onClick={() => setMapArea(place.key)}><i/><span>{language === "zh" ? place.zh : place.en}</span></button>; })}</div><small className="nasa-credit">NASA Blue Marble imagery</small></div>
          <div className="map-results"><div><small>{language === "zh" ? "当前位置" : "Now exploring"}</small><h3>{language === "zh" ? activeGlobePlace.zh : activeGlobePlace.en}</h3><p className="landmark-name">{language === "zh" ? "地标现场" : "ICONIC LANDMARK"} · {activeGlobePlace.landmark}</p><p>{language === "zh" ? "点击地球上的城市，在 Google 地图中查看地标，并发现我的现场片段。" : "Choose a city on Earth to see its landmark in Google Maps and explore my field notes."}</p></div><iframe className="google-map" title={`${activeGlobePlace.en} Google map`} src={`https://www.google.com/maps?q=${encodeURIComponent(activeGlobePlace.landmark)}&output=embed`} loading="lazy" referrerPolicy="no-referrer-when-downgrade"/><div className="map-story-grid">{mapStories.map((video, index) => <a href={video.url} target="_blank" rel="noreferrer" key={video.url}><img src={video.image} alt={displayVideoTitle(video, language, index)}/><span>{displayVideoTitle(video, language, index)}</span></a>)}</div></div>
      ...
      <section className="bookshelf section-pad" id="books"><div className="books-heading"><div><p className="eyebrow">{language === "zh" ? `博南书单 · ${allBooks.length}本` : `Bonan’s bookshelf · ${allBooks.length} books`}</p><h2>{language === "zh" ? "那些改变我提问方式的书。" : "Books that changed the way I ask questions."}</h2></div><p>{language === "zh" ? "不是排行榜，而是一组值得反复翻回去的思想工具。可以搜索书名、领导力、沟通、决策或人生。" : "Not a ranking, but a working library of ideas worth returning to. Search by title, leadership, communication, decisions, or life."}</p></div><div className="book-tools"><input type="search" value={bookQuery} onChange={(event) => { setBookQuery(event.target.value); setVisibleBooks(18); }} placeholder={language === "zh" ? "搜索书名或主题…" : "Search titles or themes…"}/><span>{filteredBooks.length} {language === "zh" ? "本书" : "books"}</span></div><div className="book-grid">{filteredBooks.slice(0, visibleBooks).map((book) => <article className="book-card" key={book.title}><BookCover title={book.title} isbn={"isbn" in book ? book.isbn : undefined}/><div><small>{book.author}</small><h3>{book.title}</h3><p>{book[language]}</p></div></article>)}</div>{visibleBooks < filteredBooks.length && <button className="load-more books-more" type="button" onClick={() => setVisibleBooks((count) => count + 18)}>{language === "zh" ? "继续翻阅" : "Explore more books"} ↓</button>}<Engagement id="books" language={language}/></section>
