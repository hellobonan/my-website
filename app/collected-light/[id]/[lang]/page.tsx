...
const previewVersion = "20260925-1";
const storyToEssay: Record<string, string> = {
  ...
  "story-23":"essay-22","story-24":"essay-23","story-25":"essay-24","story-26":"essay-25","story-27":"essay-26","story-28":"essay-27",
};
...
export async function generateMetadata({ params }: { params: Promise<{ id: string; lang: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: url, languages: { en: `${siteUrl}/collected-light/${id}/en`, "zh-CN": `${siteUrl}/collected-light/${id}/zh`, "x-default": `${siteUrl}/collected-light/${id}/en` } },
    openGraph: { title: content.title, description: content.summary, type: "article", url, siteName: "Hello Bonan", locale: lang === "zh" ? "zh_CN" : "en_US", publishedTime: content.story.date, authors: [`${siteUrl}/bonan-zhong`], images: [{ url: image, alt: content.title }] },
...
export default async function CollectedLightStory({ params }: { params: Promise<{ id: string; lang: string }> }) {
  ...
  const structuredData = {
    ...
    image: content.image ? `${siteUrl}${content.image}` : `${siteUrl}/og.png`,
    datePublished: content.story.date,
    dateModified: content.story.date,
    inLanguage: language === "zh" ? "zh-CN" : "en",
    mainEntityOfPage: url,
    author: { "@type": "Person", "@id": `${siteUrl}/#bonan-zhong`, name: "Bonan Zhong", alternateName: "钟博南", url: `${siteUrl}/bonan-zhong` },
    publisher: { "@type": "Person", "@id": `${siteUrl}/#bonan-zhong`, name: "Bonan Zhong" },
  };
