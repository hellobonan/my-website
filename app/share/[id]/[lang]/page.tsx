...
const sections = {
  featured: { anchor: "featured", en: ["The bottleneck is rarely where everyone is looking.", "A featured idea about finding the constraint that actually shapes the system.", "/og.png"], zh: ["瓶颈很少在所有人都盯着的地方。", "一篇关于寻找真正决定系统表现之约束点的精选思考。", "/og.png"] },
  ...
  books: { anchor: "books", en: ["Bonan’s bookshelf", "Books that changed the way Bonan Zhong asks questions.", "/og.png"], zh: ["博南书单", "那些改变钟博南提问方式的书。", "/og.png"] },
  "interesting-websites": { anchor: "web-discoveries", en: ["Interesting corners of the internet", "Useful websites for seeing—and building—the world differently.", "/og.png"], zh: ["有趣的网站", "重新观看与创造世界的实用网站。", "/og.png"] },
  ...
  subscribe: { anchor: "subscribe", en: ["A Note from Bonan", "Follow new essays, field notes, and useful discoveries from Hello Bonan.", "/og.png"], zh: ["博南来信", "订阅 Hello Bonan 的新文章、现场笔记与有用发现。", "/og.png"] },
} as const;
...
export async function generateMetadata({ params }: { params: Promise<{ id:string; lang:string }> }): Promise<Metadata> { const {id,lang}=await params; const item=content(id,lang); if(!item)return {}; const url=`${siteUrl}/share/${id}/${lang}`; const image=`${siteUrl}${item.image}`; return { title:`${item.title} | Hello Bonan`, description:item.description, alternates:{canonical:url}, openGraph:{title:item.title,description:item.description,url,type:"website",siteName:"Hello Bonan",images:[{url:image,alt:item.title}]},twitter:{card:"summary_large_image",title:item.title,description:item.description,images:[image]} }; }
